function loopOverIndicators(
  ...
  d3.map(indicators_metadata, function (d) {
    return d.Dimension;
  })
    .keys()
    .forEach(function (d) {
      dimensionList.push({ dimName: d });
    });

  dimensionList.forEach(function (z) {
    indicators_metadata
      .filter(function (d) {
        return d.Dimension == z.dimName;
      })
      .forEach(function (v) {
        var selectedCou =
        ...
        var compCou = document.getElementById("country_dropdown_comp").options[
        ...
        barchart(
          ...
          selectedCou,
          compCou,
          eval(z.dimName.toLowerCase()).filter(function (k) {
            return k.variable == v.Indicator_code;
          })
        );
      });
  });
...
function pearlchart(nameIndic, selectedCou, compCou, indicData) {
  var data = indicData.filter(function (d) {
    return d.value != "NA";
  });
  ...
  if (
    nameIndic != "Alignment" &&
    nameIndic != "Financing" &&
    nameIndic != "Flexiguidance" &&
    nameIndic != "Inclusiveness" &&
    nameIndic != "Coverage" &&
    nameIndic != "Quality" &&
    nameIndic != "Urgency"
  )
  ...
  dimGuides.forEach(function (d) {
    var indic2pick;
    if (nameIndic == "Coverage") indic2pick = "participationData";
    else indic2pick = nameIndic + "Data";
...
function barchart(nameIndic, selectedCou, compCou, indicData) {
  ...
  compChart
    ...
    .attr("class", "bar")
    .attr("y", function (d) {
      return yBar(Math.max(0, parseFloat(d.value)));
    })
    .attr("x", function (d) {
      return xBar(d.Country);
    })
    .attr("height", function (d) {
      return Math.abs(yBar(parseFloat(d.value)) - yBar(0));
    })
